...
extension Color {
    ...
    static let tfAccent = Color(uiColor: UIColor(hex: 0x1B4DFF))

    /// Muted sapphire tint for backgrounds.
    static let tfAccentSoft = Color(uiColor: UIColor.tfDynamic(
    ...
    static let tfSuccess  = Color(uiColor: UIColor.tfDynamic(
        light: UIColor(hex: 0x1F9D55),
        dark:  UIColor(hex: 0x4FBE7A)
    ))
    static let tfWarning  = Color(uiColor: UIColor.tfDynamic(
        light: UIColor(hex: 0xB58100),
        dark:  UIColor(hex: 0xE0AC4A)
    ))
    static let tfDanger   = Color(uiColor: UIColor.tfDynamic(
        light: UIColor(hex: 0xC24545),
        dark:  UIColor(hex: 0xE07070)
    ))
    static let tfCritical = Color(uiColor: UIColor.tfDynamic(
    ...
    static func statusColor(for code: Int?) -> Color {
        guard let code else { return .tfFog }
        switch code {
        case 200..<300: return .tfSuccess
        case 300..<400: return .tfWarning
        case 400..<500: return .tfDanger
        case 500..<600: return .tfCritical
        default:        return .tfFog
        }
    }
    ...
    static func methodColor(for method: String) -> Color {
        ...
        case "GET":     return .tfAccent                                  // sapphire
        case "POST":    return .tfSuccess                                 // muted green
        case "PUT":     return .tfWarning                                 // muted amber
        case "PATCH":   return Color(uiColor: UIColor(hex: 0xC97A00))     // burnt amber
        case "DELETE":  return .tfDanger                                  // muted red
    ...
    static let jsonKey    = Color(uiColor: UIColor.tfDynamic(
        light: UIColor(hex: 0x6E40A0),
        dark:  UIColor(hex: 0xB18EE0)
    ))
    static let jsonString = Color(uiColor: UIColor.tfDynamic(
        light: UIColor(hex: 0x1F7A2D),
        dark:  UIColor(hex: 0x6FCB7E)
    ))
    static let jsonNumber = Color(uiColor: UIColor.tfDynamic(
        light: UIColor(hex: 0x1B4DFF),
        dark:  UIColor(hex: 0x6F8FFF)
    ))
    static let jsonBool   = Color(uiColor: UIColor.tfDynamic(
        light: UIColor(hex: 0xB55A00),
        dark:  UIColor(hex: 0xE0995C)
    ))
    static let jsonNull   = Color.tfFog
}
...
extension UIColor {
    ...
    static func tfDynamic(light: UIColor, dark: UIColor) -> UIColor {
        UIColor { traits in
